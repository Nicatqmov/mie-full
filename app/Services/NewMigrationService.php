...
class NewMigrationService
{
    public function generateAndRunNewMigration($projectID)
    {
        try {
            $entities = Entity::where('project_id', $projectID)->where('is_new', true)->get();
            $createdMigrations = [];
    ...
    private function createMigrationFile($tableName, $entity, $projectID)
    {
        ...
        $fields = Fields::where('entity_id', $entity->id)->with('dataType')->get();
        $fieldDefinitions = '';
